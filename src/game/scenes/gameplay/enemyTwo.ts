...
export const createEnemyTwo = (color: number, fxLayer: Sprite, startY: number, finalY: number) => {
  const enemy = createSprite(assetLibrary._textures._enemy_two, [0.5, startY], [0.2, 0.2]);
  ...
  enemy._updater = (s, g, d) => {
    ...
    s._position = utils._vectorLerp([0, startY], [0.5 + (rand * 0.1), finalY + (rand * 0.05)], utils._easeCubicOut(s._lifetime * 0.4));
