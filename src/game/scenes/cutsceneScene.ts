...
export const createCutsceneScene = (cutscene: CutsceneData, onFinish: () => Scene): Scene => {
  ...
  const scene = createScene((scene, game) => {
    ...
    layerUi._addChildren([speakerTagContainer, dialogueContainer, transition]);
    ...
    const showLine = (index: number) => {
      ...
      const isUnicop = !line._speaker;
      speakerTagContainer._children = [];
      speakerTagContainer._position = utils._vectorAdd(
        isUnicop ? unicopPos : captainPos,
        [0, 0.15],
      );
    ...
    scene._updater = (_s, game, delta) => {
      ...
      const isUnicop = !currentLine._speaker;
      const isCaptain = currentLine._speaker;
      ...
      if (isUnicop) {
        ...
        unicopNode._position[1] = unicopPos[1] + utils._sin(game._worker._ticks * 10) * 0.01;
        unicopNode._rotation = utils._sin(game._worker._ticks * 6) * 0.03;
      } else {
        unicopNode._texture = assetLibrary._textures._unicorn_one;
        unicopNode._opacity = 0.55;
        unicopNode._position[1] = unicopPos[1] + utils._sin(game._worker._ticks) * 0.001;
      }

      if (isCaptain) {
        captainNode._opacity = 1;
        captainNode._position[1] = captainPos[1] + utils._sin(game._worker._ticks * 10) * 0.01;
        captainNode._rotation = utils._sin(game._worker._ticks * 6) * 0.03;
      } else {
        captainNode._opacity = 0.55;
        captainNode._position[1] = captainPos[1] + utils._sin(game._worker._ticks) * 0.001;
